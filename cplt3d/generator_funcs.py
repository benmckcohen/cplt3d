...
def compute_min_resolution(pts,vals,min_x,max_x,min_y,max_y,min_z,max_z,
             min_resolution = 0,max_resolution = 10,dist = None,focus = 'slope',
             **kwargs):
...
def compute_bound_from_dist(dist,optimize,max_resolution):
    ...
    for i in range(0,len(dist)):
        ...
        # else:
        #     print(f'    -> currently at {total_boxes} boxes >= upper limit {bins}, forcing this layer to be 0% bound')
        #     to_bind = 1
...
def tree_generator(interpolator):
    ...
    def wrap(pts,vals,min_x,max_x,min_y,max_y,min_z,max_z,
             min_resolution = None,max_resolution = None,dist = None,focus = 'slope',
             **kwargs):
        ...
        if min_resolution is None:
            ...
            min_resolution = compute_min_resolution(pts,vals,min_x,max_x,min_y,max_y,min_z,max_z,
                            min_resolution = min_resolution,max_resolution = max_resolution,dist = dist,focus = focus,
                            **kwargs)
...
def uniform_nearest_interpolator(pts,vals,X,Y,Z,dX,dY,dZ):
    '''A Method for Plotting a 3d Function in the cubic style. 
    
    Interpolates to the centers of each cube using nearest neighbor interpolation.
...
def uniform_linear_interpolator(pts,vals,X,Y,Z,dX,dY,dZ):
    '''A 3d Function in the cubic style. 
    
    Interpolates to the centers of each cube using linear interpolation. Note that this is much slower than nearest neighbor interpolation...
